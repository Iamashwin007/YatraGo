...
public class BookingDAO {
    ...
    public boolean createBooking(BookingModel booking, ArrayList<BookingSeatModel> seats) {
        ...
        try (Connection con = DbConnection.getConnection()) {
            ...
            // 1. Insert the booking row and capture the generated id
            try (PreparedStatement bookingPs =
                ...
                try (ResultSet keys = bookingPs.getGeneratedKeys()) {
                    if (keys.next()) {
                        ...
                        // stamp the booking id onto every seat before inserting
            ...
            // 2. Insert each seat row using batch for efficiency
            try (PreparedStatement seatPs = con.prepareStatement(seatSql)) {
                for (BookingSeatModel seat : seats) {
                    ...
                    // passengerAge is nullable — must use setNull when absent
    ...
    // ══════════════════════════════════════════════════════════════════════
    //  METHOD 2 — getBookingsByUser
    //  Returns all bookings for a user with route + bus info via JOINs.
    //  The JSP gets everything in one call — no extra queries needed.
    ...
    public BookingModel getBookingById(int bookingId) {
        ...
        try (Connection con = DbConnection.getConnection()) {
            ...
            // 1. Fetch the booking with joined data
            ...
            // 2. Fetch its seat rows and attach to the booking
            ...
            try (PreparedStatement seatPs = con.prepareStatement(seatsSql)) {
                ...
                while (srs.next()) {
                    ...
                    seat.setPassengerAge((Integer) srs.getObject("passenger_age")); // handles NULL
    ...
    // ══════════════════════════════════════════════════════════════════════
    //  METHOD 4 — cancelBooking
    //  Sets status = 'cancelled' AND restores seats to the schedule.
    //  Both updates happen in ONE transaction.
    //  Returns false if already cancelled, not found, or DB error.
    //
    //  WHY two connections: MySQL forbids a subquery that reads and writes
    //  the same table in one statement, so we pre-fetch passenger_count
    //  and schedule_id in a separate read, then do the write transaction.
    // ══════════════════════════════════════════════════════════════════════
    public boolean cancelBooking(int bookingId) {

        // Step A — pre-fetch values we need before the write transaction
        int passengerCount;
        int scheduleId;
        ...
        // Step B — write transaction: cancel booking + restore seats
        String cancelSql =
        ...
        try (Connection con = DbConnection.getConnection()) {
            ...
            try (PreparedStatement cancelPs = con.prepareStatement(cancelSql)) {
                ...
                if (updated == 0) {
                    // Race condition — another request already cancelled it
                    con.rollback();
                    return false;
                }
            }

            // 2. Restore available_seats on the schedule
    ...
    //  Maps one ResultSet row to a BookingModel.
    //  Shared by getBookingsByUser and getBookingById — keeps them DRY.
    // ══════════════════════════════════════════════════════════════════════
